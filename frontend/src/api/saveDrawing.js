...
// ✅ Save Drawing
export const saveDrawing = async (userId, shapes, title) => {
    try {
        const res = await fetch(`${API_BASE_URL}/api/draw/save`, {
            method: "POST",
...
export const getUserDrawings = async (userId) => {
    ...
    try {
        const res = await fetch(`${API_BASE_URL}/api/draw/user/${userId}`, {
            method: "GET",
            credentials: "include", // for JWT cookies
        });
...
export const deleteDrawing = async (drawingId) => {
    try {
        const res = await fetch(`${API_BASE_URL}/api/draw/${drawingId}`, {
            method: "DELETE",
            credentials: "include", // for JWT cookies
        });
